...
require "bunni/consumer_work_pool"

require "bunni/exchange"
require "bunni/queue"

require "bunni/delivery_info"
require "bunni/return_info"
require "bunni/message_properties"

if defined?(JRUBY_VERSION)
  require "bunni/concurrent/linked_continuation_queue"
else
  require "bunni/concurrent/continuation_queue"
end
...
module Bunni
  ...
  # ## Opening Channels
  #
  # Channels can be opened either via `Bunni::Session#create_channel` (sufficient in the majority
  # of cases) or by instantiating `Bunni::Channel` directly:
  #
  #     conn = Bunni.new
  #     conn.start
  #
  ...
  # ## Closing Channels
  #
  # Channels are closed via {Bunni::Channel#close}. Channels that get a channel-level exception are
  # closed, too. Closed channels can no longer be used. Attempts to use them will raise
  # {Bunni::ChannelAlreadyClosed}.
  #
  #     ch = conn.create_channel
  #     ch.close
  #
  # ## Higher-level API
  #
  # Bunni offers two sets of methods on {Bunni::Channel}: known as higher-level and lower-level
  # APIs, respectively. Higher-level API mimics {http://rubyamqp.info amqp gem} API where
  # exchanges and queues are objects (instance of {Bunni::Exchange} and {Bunni::Queue}, respectively).
  ...
  #
  # * {Bunni::Channel#queue} is used to declare queues. The rest of the API is in {Bunni::Queue}.
  #
  #
  # ### Exchange Operations In Higher-level API
  #
  # * {Bunni::Channel#topic} declares a topic exchange. The rest of the API is in {Bunni::Exchange}.
  # * {Bunni::Channel#direct} declares a direct exchange.
  # * {Bunni::Channel#fanout} declares a fanout exchange.
  # * {Bunni::Channel#headers} declares a headers exchange.
  # * {Bunni::Channel#default_exchange}
  # * {Bunni::Channel#exchange} is used to declare exchanges with type specified as a symbol or string.
  #
  #
  ...
  # or rejects previously consumed ones). This setting is per channel and controlled via {Bunni::Channel#prefetch}.
  #
  #
  # ## Channel IDs
  #
  # Channels are identified by their ids which are integers. Bunni takes care of allocating and
  ...
  #
  # With Bunni, channel-level exceptions are raised as Ruby exceptions, for example,
  # {Bunni::NotFound}, that provide access to the underlying `channel.close` method
  # information.
  #
  ...
  # @see http://rubybunni.info/articles/getting_started.html Getting Started with RabbitMQ Using Bunni
  # @see http://rubybunni.info/articles/queues.html Queues and Consumers
  # @see http://rubybunni.info/articles/exchanges.html Exchanges and Publishing
  # @see http://rubybunni.info/articles/error_handling.html Error Handling and Recovery Guide
  class Channel
    ...
    attr_accessor :id
    # @return [Bunni::Session] AMQP connection this channel was opened on
    attr_reader :connection
    # @return [Symbol] Channel status (:opening, :open, :closed)
    attr_reader :status
    # @return [Bunni::ConsumerWorkPool] Thread pool delivered messages are dispatched to.
    attr_reader :work_pool
    # @return [Integer] Next publisher confirmations sequence index
    attr_reader :next_publish_seq_no
    # @return [Hash<String, Bunni::Queue>] Queue instances declared on this channel
    attr_reader :queues
    # @return [Hash<String, Bunni::Exchange>] Exchange instances declared on this channel
    ...
    # @return [Hash<String, Bunni::Consumer>] Consumer instances declared on this channel
    ...
    # @param [Bunni::Session] connection AMQP 0.9.1 connection
    # @param [Integer] id Channel id, pass nil to make Bunni automatically allocate it
    # @param [Bunni::ConsumerWorkPool] work_pool Thread pool for delivery processing, by default of size 1
    def initialize(connection = nil, id = nil, work_pool = ConsumerWorkPool.new(1))
      ...
      @recoveries_counter = Bunni::Concurrent::AtomicFixnum.new(0)
    ...
    # {Bunni::Queue}, {Bunni::Exchange} and {Bunni::Consumer} instances.
    # @api public
    def close
    ...
    # @return [Bunni::Session] Connection this channel was opened on
    def client
      @connection
    end
    ...
    #
    # @return [Bunni::Exchange] Exchange instance
    # @see http://rubybunni.info/articles/exchanges.html Exchanges and Publishing guide
    # @see http://rubybunni.info/articles/extensions.html RabbitMQ Extensions to AMQP 0.9.1 guide
    # @api public
    ...
    #
    # @return [Bunni::Queue] Queue that was declared or looked up in the cache
    # @see http://rubybunni.info/articles/queues.html Queues and Consumers guide
    # @see http://rubybunni.info/articles/extensions.html RabbitMQ Extensions guide
    # @api public
    def queue(name = AMQ::Protocol::EMPTY_STRING, opts = {})
      q = find_queue(name) || Bunni::Queue.new(self, name, opts)
    ...
    #
    # @return [Bunni::Queue] Queue that was declared
    # @see #queue
    # @api public
    ...
    # @see Bunni::Channel#ack
    # @see Bunni::Channel#nack
    # @see http://rubybunni.info/articles/queues.html Queues and Consumers guide
    # @api public
    ...
    # Rejects a message. A rejected message can be requeued or
    # dropped by RabbitMQ. This method is similar to {Bunni::Channel#reject} but
    ...
    #
    # @param [String] payload Message payload. It will never be modified by Bunni or RabbitMQ in any way.
    ...
    #
    # @example Using Bunni::Channel#basic_get with manual acknowledgements
    #   conn = Bunni.new
    #   conn.start
    #   ch   = conn.create_channel
    #   # here we assume the queue already exists and has messages
    #   delivery_info, properties, payload = ch.basic_get("bunni.examples.queue1", :manual_ack => true)
    ...
    def basic_qos(count, global = false)
      ...
      Bunni::Timeout.timeout(wait_on_continuations_timeout, ClientTimeout) do
    ...
    #   delivery_info, properties, payload = ch.basic_get("bunni.examples.queue3", :manual_ack => true)
    #   ch.basic_reject(delivery_info.delivery_tag, true)
    #
    # @see Bunni::Channel#basic_nack
    # @see http://rubybunni.info/articles/queues.html Queues and Consumers guide
    # @api public
    ...
    #   ch    = conn.create_channel
    #   # we assume the queue exists and has messages
    #   _, _, payload1 = ch.basic_get("bunni.examples.queue3", :manual_ack => true)
    #   _, _, payload2 = ch.basic_get("bunni.examples.queue3", :manual_ack => true)
    #   delivery_info, properties, payload3 = ch.basic_get("bunni.examples.queue3", :manual_ack => true)
    ...
    # Rejects or requeues messages just like {Bunni::Channel#basic_reject} but can do so
    ...
    #
    # @param [String, Bunni::Queue] queue Queue to consume from
    # @param [String] consumer_tag Consumer tag (unique identifier), generated by Bunni by default
    ...
    # Registers a consumer for queue as {Bunni::Consumer} instance.
    #
    # @param [Bunni::Consumer] consumer Consumer to register. It should already have queue name, consumer tag
    ...
    #                                          exist, {Bunni::NotFound} will be raised.
    #
    ...
    # @see http://rubybunni.info/articles/queues.html Queues and Consumers guide
    # @see http://rubybunni.info/articles/bindings.html Bindings guide
    # @api public
    ...
    # @see http://rubybunni.info/articles/echanges.html Exchanges and Publishing guide
    # @api public
    ...
    # @api plugin
    def generate_consumer_tag(name = "bunni")
    ...
    def handle_method(method)
      ...
      when AMQ::Protocol::Channel::CloseOk then
        ...
        raise "Do not know how to handle #{method.inspect} in Bunni::Channel#handle_method"
      end
    end

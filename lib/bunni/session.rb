...
require "bunni/transport"
require "bunni/channel_id_allocator"
require "bunni/heartbeat_sender"
require "bunni/reader_loop"
require "bunni/authentication/credentials_encoder"
require "bunni/authentication/plain_mechanism_encoder"
require "bunni/authentication/external_mechanism_encoder"
...
module Bunni
  ...
  # @see http://rubybunni.info/articles/connecting.html Connecting to RabbitMQ guide
  class Session
    ...
    DEFAULT_CLIENT_PROPERTIES = {
      ...
      :version      => Bunni::VERSION,
      :information  => "http://rubybunni.info",
    }
    ...
    # @option connection_string_or_opts [Integer] :continuation_timeout (4000) Timeout for client operations that expect a response (e.g. {Bunni::Queue#get}), in milliseconds.
    ...
    #
    # @see http://rubybunni.info/articles/connecting.html Connecting to RabbitMQ guide
    # @see http://rubybunni.info/articles/tls.html TLS/SSL guide
    # @api public
    ...
    #
    # @return [Bunni::Channel] Newly opened channel
    def create_channel(n = nil, consumer_pool_size = 1)
      ...
      @channel_mutex.synchronize do
        if n && (ch = @channels[n])
          ...
          ch = Bunni::Channel.new(self, n, ConsumerWorkPool.new(consumer_pool_size || 1))
          ch.open
          ch
        end
      end
    ...
    # Parses an amqp[s] URI into a hash that {Bunni::Session#initialize} accepts.
    #
    ...
    def close_all_channels
      @channels.reject {|n, ch| n == 0 || !ch.open? }.each do |_, ch|
        Bunni::Timeout.timeout(@transport.disconnect_timeout, ClientTimeout) { ch.close }
      end
    end
    ...
    # Exposed primarily for Bunni::Channel
    #
    ...
    # timeout control. Exposed primarily for Bunni::Channel.
    #
    # @raise [ConnectionClosedError]
    # @private
    ...
    def open_connection
      ...
      if response.is_a?(AMQ::Protocol::Connection::Close)
        ...
        raise Bunni::AuthenticationFailureError.new(self.user, self.vhost, self.password.size)
      end
    ...
    def ignoring_io_errors(&block)
      begin
        block.call
      rescue AMQ::Protocol::EmptyResponseError, IOError, SystemCallError, Bunni::NetworkFailure => _
        # ignore
      end
    end
  end # Session

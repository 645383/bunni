  ...
  class Transport
    ...
    if defined?(JRUBY_VERSION)
      ...
      def write(data)
        ...
        begin
        ...
        rescue SystemCallError, Timeout::Error, Bunni::ConnectionError, IOError => e
          ...
          if @session.automatically_recover?
            ...
            @session_thread.raise(Bunni::NetworkFailure.new("detected a network failure: #{e.message}", e))
          end
        end
      end
    else
    ...
    def write_without_timeout(data)
      begin
        ...
        @socket.flush
      rescue SystemCallError, Bunni::ConnectionError, IOError => e
        close
    ...
    def self.reacheable?(host, port, timeout)
      begin
        s = Bunni::SocketImpl.open(host, port,
    ...
    def initialize_socket
      begin
        @socket = Bunni::SocketImpl.open(@host, @port,
          :keepalive      => @opts[:keepalive],
          :connect_timeout => @connect_timeout)
      rescue StandardError, ClientTimeout => e
        @status = :not_connected
        raise Bunni::TCPConnectionFailed.new(e, self.hostname, self.port)
      end
    ...
    def post_initialize_socket
      @socket = if uses_tls? and !@socket.is_a?(Bunni::SSLSocketImpl)
    ...
    def wrap_in_tls_socket(socket)
      raise ArgumentError, "cannot wrap nil into TLS socket, @tls_context is nil. This is a Bunni bug." unless socket
      raise "cannot wrap a socket into TLS socket, @tls_context is nil. This is a Bunni bug." unless @tls_context

      s = Bunni::SSLSocketImpl.new(socket, @tls_context)
      s.sync_close = true
      s
    end
